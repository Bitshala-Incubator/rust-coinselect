...
use bitcoin::{
     Amount, OutPoint, ScriptBuf, Sequence, TxIn, Witness ,TxOut, Transaction, absolute::LockTime, Txid, hex::FromHex, transaction
};
use rust_coinselect::{
...
use std::fs;
use std::{error::Error, fmt, str::FromStr, fs::File, path::Path, io::Read, collections::HashSet};
...
fn json_to_txin(filedata: &str) -> Result<Vec<TxIn>, Box<dyn std::error::Error>> {
    ...
    for tx_inp in tx_in_json_vec {
        ...
        let witness: Vec<&str> = tx_inp.witness.iter()
                    .map(|w| &w[..])
                    .collect();
...
fn create_transaction(txinput:Vec<TxIn>, txoutput:Vec<TxOut>) -> Result<Transaction, Box<dyn std::error::Error>>{
...
fn compose_transaction(inputs:Vec<TxIn>, outputs:Vec<TxOut>)-> Result<Vec<Transaction>, Box<dyn std::error::Error>>{
            ...
            for inputs_combination in inputs.iter().combinations(inputs_size) {
                for outputs_combination in outputs.iter().combinations(outputs_size){
                    let tx = create_transaction(
                        inputs_combination.clone().into_iter().cloned().collect::<Vec<_>>(),
    ...
    info!("Successfully created {:?} transactions", transactions_vec.len());
    Ok(transactions_vec)
}

fn create_outputgroup(tx: Vec<Transaction>) -> Result<Vec<OutputGroup>, Box<dyn std::error::Error>> {
    ...
    info!("Successfully created {:?} outputgroups", output_group_vec.len());
    Ok(output_group_vec)
}
...
fn create_select_options() -> Result<Vec<CoinSelectionOpt>, Box<dyn std::error::Error>> {
    ...
    info!("Successfully created {:?} selection options", coin_select_options_vec.len());
